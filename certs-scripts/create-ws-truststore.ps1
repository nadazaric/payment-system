...
$wsTruststore = Join-Path $RootDir "web-shop\back\src\main\resources\certs\truststore.p12"
$pspCert = Join-Path $TempDir "psp-back-cert.cer"

if ((Test-Path $wsTruststore) -and $Force) {
    Remove-Item $wsTruststore -Force
    Write-Log "INFO" "Removed existing ws-back truststore: $wsTruststore"
}
...
Import-CertificateToTruststore `
    ...
    -TruststorePath $wsTruststore `
    -TrustAlias "pspbackend" `
    -Keytool $KeytoolPath

Write-Log "INFO" "ws-back truststore is ready. It trusts psp-back."
