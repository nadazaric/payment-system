...
$pspKeystore = Join-Path $RootDir "psp\back\src\main\resources\certs\keystore.p12"
$bankKeystore = Join-Path $RootDir "bank\back\src\main\resources\certs\keystore.p12"
$pluginTruststore = Join-Path $RootDir "psp\plugin\bank-payment-plugin\src\main\resources\certs\truststore.p12"
$pspCert = Join-Path $TempDir "psp-back-cert.cer"
$bankCert = Join-Path $TempDir "bank-back-cert.cer"

if ((Test-Path $pluginTruststore) -and $Force) {
    Remove-Item $pluginTruststore -Force
    Write-Log "INFO" "Removed existing bank-payment-plugin truststore: $pluginTruststore"
}

Import-CertificateToTruststore `
    -SourceKeystorePath $pspKeystore `
    -SourceAlias "pspbackend" `
    -CertificateFile $pspCert `
    -TruststorePath $pluginTruststore `
    -TrustAlias "pspbackend" `
    -Keytool $KeytoolPath

Import-CertificateToTruststore `
    -SourceKeystorePath $bankKeystore `
    -SourceAlias "bankbackend" `
    -CertificateFile $bankCert `
    -TruststorePath $pluginTruststore `
    -TrustAlias "bankbackend" `
    -Keytool $KeytoolPath

Write-Log "INFO" "bank-payment-plugin truststore is ready. It trusts psp-back and bank-back."
