...
$pspTruststore = Join-Path $RootDir "psp\back\src\main\resources\certs\truststore.p12"
$pluginCert = Join-Path $TempDir "bank-payment-plugin-cert.cer"

if ((Test-Path $pspTruststore) -and $Force) {
    Remove-Item $pspTruststore -Force
    Write-Log "INFO" "Removed existing psp-back truststore: $pspTruststore"
}
...
Import-CertificateToTruststore `
    ...
    -TruststorePath $pspTruststore `
    -TrustAlias "bankpaymentplugin" `
    -Keytool $KeytoolPath

Write-Log "INFO" "psp-back truststore is ready. It trusts bank-payment-plugin."
